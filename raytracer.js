...
if (isect != null) {
	...
	if ((mat.kr != null || mat.kt != null)) {
		// mirror
		if (mat.kr && depth < maxDepth) {
			let V = ray.d.clone().negate();
			let N = isect.normal.clone().normalize();
			if (N.dot(V) > 0) {
				let reflDir = reflect(V, N);
				let reflRay = new Ray(isect.position.clone().addScaledVector(reflDir, 0.0001), reflDir);
				let reflColor = raytracing(reflRay, depth + 1);
				color.add(reflColor.multiply(mat.kr));
			}
		}
		// glass
		if (mat.kt && depth < maxDepth) {
			let refrDir = refract(ray.d.clone(), isect.normal.clone(), mat.ior);
			if (refrDir != null) {
				let refrRay = new Ray(isect.position.clone().addScaledVector(refrDir, 0.0001), refrDir);
				let refrColor = raytracing(refrRay, depth + 1);
				color.add(refrColor.multiply(mat.kt));
			}
		}
	} else {
		// Ambient
		color.add(ambientLight.clone().multiply(isect.material.ka));
		color.add(shading(ray, isect)); //this part of if-else is implemented in shading function
	}
	return color;
}
...
function shading(ray, isect) {
	...
	for (let light of lights) {
		let lightSample = light.getLight(isect.position);

		// Shadow
		let shadowRay = new Ray(
			isect.position.clone().addScaledVector(lightSample.direction, 0.0001),
			lightSample.direction.clone()
		);
		let shadowIsect = rayIntersectScene(shadowRay);
		if (shadowIsect && shadowIsect.t < isect.position.distanceTo(lightSample.position)) {
			continue;
		}

		// Diffuse
		let N = isect.normal.clone().normalize();
		let L = lightSample.direction.clone().normalize();
		let NdotL = Math.max(N.dot(L), 0);
		let diffuse = isect.material.kd.clone().multiply(lightSample.intensity).multiplyScalar(NdotL);
		color.add(diffuse);

		// Specular
		if (isect.material.ks != null) {
			let V = ray.o.clone().sub(isect.position).normalize(); 
			let N = isect.normal.clone().normalize();
			if (N.dot(V) > 0) { 
				let R = reflect(L, N);
				let RdotV = Math.max(R.dot(V), 0);
				let specularFactor = Math.pow(RdotV, isect.material.p);
				let specular = isect.material.ks.clone().multiply(lightSample.intensity).multiplyScalar(specularFactor);
				color.add(specular);
			}
		}
	}
